...
const BackgroundCanvas: React.FC = () => {
  ...
  const setup = (p5: p5Types, canvasParentRef: Element) => {
    ...
    createDots(p5);
    createMetaballs(p5);
    initializeConstellations(p5);
  };

  const initializeConstellations = (p5: p5Types) => {
  ...
  const createDots = (p5: p5Types) => {
    ...
    for (let x = -GRID_SIZE; x < p5.width + GRID_SIZE; x += GRID_SIZE) {
      for (let y = -GRID_SIZE - GRID_SIZE / 2; y < p5.height + GRID_SIZE; y += GRID_SIZE) {
  ...
  const updateMetaballs = (p5: p5Types) => {
    ...
    const attractionStrength = Math.max(0, 1 - timeSinceLastMove / MOUSE_ATTRACTION_TAPER_DURATION);

    metaballs.current.forEach((metaball, index) => {
  ...
  const updateMetaballPosition = (p5: p5Types, metaball: Metaball, attractionStrength: number) => {
    ...
    if (attractionStrength > 0) {
      ...
      if (distToMouse < METABALL_MOUSE_ATTRACTION_RADIUS) {
        const baseAttractionForce = (1 - distToMouse / METABALL_MOUSE_ATTRACTION_RADIUS) * METABALL_MOUSE_ATTRACTION_STRENGTH;
  ...
  const handleMetaballRepulsion = (p5: p5Types, metaball: Metaball, index: number) => {
    for (let i = index + 1; i < metaballs.current.length; i++) {
      ...
      if (distance < METABALL_REPULSION_RADIUS) {
        const repulsionForce = (1 - distance / METABALL_REPULSION_RADIUS) * METABALL_REPULSION_STRENGTH;
  ...
  const handleMetaballScreenCollision = (p5: p5Types, metaball: Metaball) => {
    if (metaball.x - metaball.radius < 0 || metaball.x + metaball.radius > p5.width) {
      metaball.vx *= -METABALL_BOUNCE_DECAY;
      metaball.x = p5.constrain(metaball.x, metaball.radius, p5.width - metaball.radius);
    }
    if (metaball.y - metaball.radius < 0 || metaball.y + metaball.radius > p5.height) {
      metaball.vy *= -METABALL_BOUNCE_DECAY;
      metaball.y = p5.constrain(metaball.y, metaball.radius, p5.height - metaball.radius);
    }
  };
  ...
  const updateSectors = (p5: p5Types) => {
    ...
    sectors.current = Array(rows).fill(null).map(() => Array(cols).fill(null).map(() => []));
  ...
  const updateDots = (p5: p5Types) => {
    ...
    const mouseSpeed = Math.sqrt(rawMouseVelocity.x ** 2 + rawMouseVelocity.y ** 2);
  ...
  const updateDotPosition = (p5: p5Types, dot: Dot, normalizedMouseSpeed: number) => {
    let fieldForceX = 0;
    let fieldForceY = 0;
    ...
    if (distanceToMouse < MOUSE_INFLUENCE_RADIUS) {
      const mouseEffect = (1 - distanceToMouse / MOUSE_INFLUENCE_RADIUS) * normalizedMouseSpeed * MAX_MOUSE_EFFECT;
  ...
  const updateConstellations = (p5: p5Types) => {
    constellations.current.forEach(constellation => {
      ...
      if (p5.random() < NODE_ADD_CHANCE && constellation.nodes.length < MAX_NODES_PER_CONSTELLATION) {
  ...
  const addNodeToConstellation = (p5: p5Types, constellation: Constellation) => {
    const { metaball } = constellation;
    const angle = p5.random(p5.TWO_PI);
    ...
    if (closestDot) {
      const newNode: HighlightNode = {
        dot: closestDot,
        radius: 0,
        targetRadius: p5.random(MAX_HIGHLIGHT_RADIUS * 0.5, MAX_HIGHLIGHT_RADIUS),
        opacity: 0
      };
      constellation.nodes.push(newNode);

      if (constellation.nodes.length > 1) {
        const nearestNode = findNearestNode(p5, newNode, constellation.nodes.slice(0, -1));
        if (nearestNode) {
  ...
  const ensureConnectivity = (p5: p5Types, constellation: Constellation) => {
    ...
    if (connectedComponents.length > 1) {
      for (let i = 1; i < connectedComponents.length; i++) {
        const nearestPair = findNearestNodesBetweenComponents(p5, connectedComponents[0], connectedComponents[i]);
        if (nearestPair) {
  ...
  const dfs = (node: HighlightNode, visited: Set<HighlightNode>, component: HighlightNode[], constellation: Constellation) => {
    visited.add(node);
    component.push(node);
    ...
    constellation.lines.forEach(line => {
      ...
      if (line.start === node.dot) {
        ...
        neighbor = constellation.nodes.find(n => n.dot === line.start) || null;
      }
  ...
  const findNearestNodesBetweenComponents = (p5: p5Types, component1: HighlightNode[], component2: HighlightNode[]): [HighlightNode, HighlightNode] | null => {
    let minDist = Infinity;
    let nearestPair: [HighlightNode, HighlightNode] | null = null;

    component1.forEach(node1 => {
      component2.forEach(node2 => {
        const dist = p5.dist(node1.dot.x, node1.dot.y, node2.dot.x, node2.dot.y);
        if (dist < minDist) {
          minDist = dist;
  ...
  const findNearestNode = (p5: p5Types, node: HighlightNode, nodes: HighlightNode[]) => {
    let nearestNode = null;
    let minDist = Infinity;

    nodes.forEach(otherNode => {
      const dist = p5.dist(node.dot.x, node.dot.y, otherNode.dot.x, otherNode.dot.y);
      if (dist < minDist) {
        minDist = dist;
  ...
  const addLine = (constellation: Constellation, start: HighlightNode, end: HighlightNode) => {
    const existingLine = constellation.lines.find(line => 
    ...
    constellations.current.forEach(constellation => {
      ...
      constellation.lines.forEach(line => {
        ...
        if (startNode && endNode) {
          const lineOpacity = Math.min(startNode.opacity, endNode.opacity) * line.progress;
  ...
  const renderDebugInfo = (p5: p5Types) => {
    ...
    p5.text(`Total Constellation Nodes: ${constellations.current.reduce((sum, c) => sum + c.nodes.length, 0)}`, 10, 40);
    p5.text(`Total Constellation Lines: ${constellations.current.reduce((sum, c) => sum + c.lines.length, 0)}`, 10, 60);
  };
  ...
  const renderDebugMetaball = (p5: p5Types, constellation: Constellation) => {
    ...
    p5.stroke(255, 0, 0);
    p5.strokeWeight(1);
    p5.circle(constellation.metaball.x, constellation.metaball.y, constellation.metaball.radius * 2);
    p5.circle(constellation.metaball.x, constellation.metaball.y, METABALL_INFLUENCE_RADIUS * 2);
  };
